...
impostatitolo(f"ErencanDeniz Patlatma Araci {VERSIONETOOL} - Emoji Silme")
clear()
...
async def on_ready():
    ...
    for emojidiocane in serverr.emojis:
        try:
            ...
            print(f'{y}[{b}ErencanDeniz{y}]{g} Sildim {emojidiocane} {w}')
        except Forbidden:
            print(f'{y}[{b}ErencanDeniz{y}]{r} Silemedim {emojidiocane} [Izin yok]{w}')
        except:
            print(f'{y}[{b}ErencanDeniz{y}]{r} Sildim {emojidiocane}{w}')
