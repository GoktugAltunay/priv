    ...
    for channel in channels:
        for user in [x["username"]+"#"+x["discriminator"] for x in channel["recipients"]]:
            try:
                ...
                print(f"{y}[{Fore.LIGHTGREEN_EX }!{y}]{w} Gonderildi: {user}")
            except Exception as e:
                print(f"{y}[{Fore.LIGHTRED_EX }!{y}]{w} The following error has been encountered and is being ignored: {e}")

print(f"{y}[{w}+{y}]{w} Spam atmak istediginiz tokeni girin")
...
if validityTest.status_code != 200:
    ...
    input(f"\n{y}[{b}#{y}]{w} Cikmak icin ENTERa basin")
    main()
print(f"\n{y}[{w}+{y}]{w} Tum DM Kutunuza Gonderilecek Mesaji Yazin")
message = str(input(f"{y}[{b}#{y}]{w} Mesaj: "))
clear()
processes = []
